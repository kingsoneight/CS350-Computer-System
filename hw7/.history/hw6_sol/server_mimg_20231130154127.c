	...
	do
	{
		...
		if (in_bytes > 0)
		{
			...
			if (req->request.img_op == IMG_REGISTER)
			{
				...
				/* inter-threaded synchronization */
				int my_arrive = arrives[req->request.img_id];
				arrives[req->request.img_id]++;

				wait(sem_arr[req->request.img_id]);

				while (opt[req->request.img_id] != my_arrive)
				{
					sem_post(sem_arr[req->request.img_id]);
					sem_wait(sem_arr[req->request.img_id]);
				}

				opt[req->request.img_id]++;
				clock_gettime(CLOCK_MONOTONIC, &req->start_timestamp);
