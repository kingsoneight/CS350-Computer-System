...
/* Mutex protecting the image pool*/
sem_t *pool_mutex;
...
/* Global array of semaphores for images */
sem_t **sem_arr;

/* Global arrays storing arrives and current opt for different images (used for ensuring ordering of image operations)*/
int *arrives;
int *opt;
...
/* Mutex protecting the connection socket */
sem_t *socket_mutex;
...
void register_new_image(int conn_socket, struct request *req)
{
	...
	// sem_wait(pool_mutex);
	/* Reallocate array of image pointers, and the arrays for semaphores and arrivals */
	images = realloc(images, image_count * sizeof(struct image *));

	sem_arr = realloc(sem_arr, image_count * sizeof(sem_t *));
	sem_arr[image_count - 1] = malloc(sizeof(sem_t));

	if (sem_init(sem_arr[image_count - 1], 0, 1) != 0)
	{
	...
	arrives = realloc(arrives, image_count * sizeof(int));
	arrives[image_count - 1] = 0;
	opt = realloc(opt, image_count * sizeof(int));
	opt[image_count - 1] = 0;
...
int worker_main(void *arg)
{
	...
	while (!params->worker_done)
	{
		...
		my_arrive = arrives[img_id];
		arrives[img_id]++;

		sem_wait(sem_arr[img_id]);

		while (opt[img_id] != my_arrive)
		{
			sem_post(sem_arr[img_id]);
			sem_wait(sem_arr[img_id]);
		}
...
int main(int argc, char **argv)
{
	...
	/* Initilize image pool mutex */
	pool_mutex = (sem_t *)malloc(sizeof(sem_t));
	retval = sem_init(pool_mutex, 0, 1);
	if (retval < 0)
	{
		ERROR_INFO();
		perror("Unable to initialize pool mutex");
		return EXIT_FAILURE;
	}

	/* Initilize connection socket mutex */
