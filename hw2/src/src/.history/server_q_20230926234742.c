...
void dump_queue_status(struct queue *the_queue)
{
	...
	int size = the_queue->front > the_queue->rear ? (QUEUE_SIZE - the_queue->front + the_queue->rear + 1) : (the_queue->rear - the_queue->front + 1);
	...
	while (1)
	{
		printf("R%llu", the_queue->mes_queue[i].req.req_id);
...
int worker_main(struct worker_params *p)
{
	struct request rq;
	struct message msg;
	...
	while (1)
	{
		msg = get_from_queue(p->the_queue);
		if (msg.recei.tv_sec == -1)
		{
			printf("child thread breaks out of loop.");
			break;
		}
		...
		if (send(p->conn_socket, &rq.req_id, sizeof(uint64_t), 0) == -1)
		{
			printf("Not sent");
		}
...
void handle_connection(int conn_socket)
{
	...
	the_queue->front = -1;
	the_queue->rear = -1;
	param = (struct param *)malloc(sizeof(struct worker_params));
	param->conn_socket = conn_socket;
	param->the_queue = the_queue;
	/* Queue ready to go here. Let's start the worker thread. */

	/* IMPLEMENT HERE THE LOGIC TO START THE WORKER THREAD. */
	int res = clone(worker_main, worker_stack + STACK_SIZE, CLONE_THREAD | CLONE_VM | CLONE_SIGHAND | CLONE_FS | CLONE_FILES | CLONE_SYSVSEM, param);
