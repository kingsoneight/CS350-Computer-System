...
int worker_main(struct queue *q)
{
	...
	while (1)
	{
		...
		get_elapsed_busywait(msg.req.req_length.tv_sec, msg.req.req_length.tv_nsec);
		...
		printf("R%llu:%0.6f,%0.6f,%0.6f,%0.6f,%0.6f", rq.req_id, sent_timestamp, request_length, receipt_timestamp, completion_timestamp);
		fflush(stdout);
	}
}
