 ...
 *
 * Usage:`ld directory>/server <port_number>
 *
 * Parameters:
...
int worker_main(void *param)
{
	...
	printf("[#WORKER#] %0.6f Worker Thread Alive!\n", start_time);

	/* Enter the forever while loop. */
	while (1)
	{
		get_elapsed_busywait(1, 0);
		clock_gettime(CLOCK_MONOTONIC, &cur);

		double cur_time = (double)cur.tv_sec + ((double)cur.tv_nsec / 1e9);
		printf("[#WORKER#] %0.6f Still Alive!\n", cur_time);
...
void handle_connection(int conn_socket)
{
	...
	int res = clone(worker_main, worker_stack + STACK_SIZE, CLONE_THREAD | CLONE_VM | CLONE_SIGHAND | CLONE_FS | CLONE_FILES | CLONE_SYSVSEM, NULL);
	if (res == -1)
	{
		perror("clone failed");
	}
