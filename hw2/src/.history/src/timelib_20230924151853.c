...
uint64_t get_elapsed_busywait(long sec, long nsec)
{
	...
	uint64_t clock1, clock2, target_time, current_time;

	if (clock_gettime(CLOCK_MONOTONIC, &tp) == 0)
	{
		begin_timestamp = tp;
		get_clocks(clock1);
		target_time = begin_timestamp.tv_sec * 1e9 + begin_timestamp.tv_nsec + sec * 1e9 + nsec;
		do
		{
			clock_gettime(CLOCK_MONOTONIC, &tp);
			get_clocks(clock2);
			current_time = tp.tv_sec * 1e9 + tp.tv_nsec;
