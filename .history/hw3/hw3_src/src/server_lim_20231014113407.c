...
{
	struct queue *the_queue = (struct queue *)malloc(sizeof(struct queue) + sizeof(struct request_meta) * queue_size);
	the_queue->front = -1;
	the_queue->rear = -1;
...
int worker_main(struct worker_params *params)
{
	...
	while (1)
	{
		...
		if (send(params->conn_socket, &rq.req_id, sizeof(uint64_t), 0) == -1)
		{
			printf("Not sent");
		}
...
void handle_connection(int conn_socket, struct connection_params conn_params)
{
	...
	do
	{
		...
		ssize_t in_bytes = read(conn_socket, &(req->request), sizeof(struct request));
