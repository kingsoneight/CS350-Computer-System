	...
	while (!params->worker_done)
	{
		...
		if (req.request.img_op == IMG_ROT90CLKW)
		{
			...
			if (req.request.overwrite == 1)
			{
				...
				uint8_t errorStatus;
				params->pool[req.request.img_id] = rotate90Clockwise(params->pool[req.request.img_id], errorStatus);
		...
		if (req.request.img_op == IMG_BLUR)
		{
			...
			if (req.request.overwrite == 1)
			{
				...
				params->pool[req.request.img_id] = blurImage(params->pool[req.request.img_id]);
			}
			else if (req.request.overwrite == 0)
			{
				for (int i = 0; i < 100; i++)
		...
		if (req.request.img_op == IMG_SHARPEN)
		{
			...
			if (req.request.overwrite == 1)
			{
				...
				params->pool[req.request.img_id] = sharpenImage(params->pool[req.request.img_id]);
			}
			else if (req.request.overwrite == 0)
			{
				for (int i = 0; i < 100; i++)
		...
		if (req.request.img_op == IMG_VERTEDGES)
		{
			...
			if (req.request.overwrite == 1)
			{
				...
				params->pool[req.request.img_id] = detectVerticalEdges(params->pool[req.request.img_id]);
			}
			else if (req.request.overwrite == 0)
			{
				for (int i = 0; i < 100; i++)
		...
		if (req.request.img_op == IMG_HORIZEDGES)
		{
			...
			if (req.request.overwrite == 1)
			{
				...
				params->pool[req.request.img_id] = detectHorizontalEdges(params->pool[req.request.img_id]);
			}
			else if (req.request.overwrite == 0)
			{
				for (int i = 0; i < 100; i++)
