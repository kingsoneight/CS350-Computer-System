	...
	while (!params->worker_done)
	{
		...
		if (req.request.img_op == IMG_ROT90CLKW)
		{
			...
			if (req.request.overwrite == 1)
			{
				...
				uint8_t errorStatus;
				struct image *new = rotate90Clockwise(params->pool[req.request.img_id], errorStatus);
				deleteImage(params->pool[req.request.img_id]);
				params->pool[req.request.img_id] = new;
				...
				{
					sync_printf("Image ratation failed\n");
				}
			}
			else if (req.request.overwrite == 0)
			{
				for (int i = 0; i < 100; i++)
				{
					if (params->pool[i] == NULL)
					{

						uint8_t errorStatus;
						params->pool[i] = rotate90Clockwise(params->pool[req.request.img_id], errorStatus);
						if (errorStatus == 0)
						{
							resp.img_id = req.request.img_id;
						}
		...
		if (req.request.img_op == IMG_BLUR)
		{

			if (req.request.overwrite == 1)
			{

				struct image *new = blurImage(params->pool[req.request.img_id]);
				deleteImage(params->pool[req.request.img_id]);
				params->pool[req.request.img_id] = new;
			}
			else if (req.request.overwrite == 0)
			{
				for (int i = 0; i < 100; i++)
				{
					if (params->pool[i] == NULL)
					{
						...
						params->pool[i] = blurImage(params->pool[req.request.img_id]);
					}
				}
			}
		}

		if (req.request.img_op == IMG_SHARPEN)
		{

			if (req.request.overwrite == 1)
			{
				struct image *new = sharpenImage(params->pool[req.request.img_id]);
				deleteImage(params->pool[req.request.img_id]);
				params->pool[req.request.img_id] = new;
			}
			else if (req.request.overwrite == 0)
			{
				for (int i = 0; i < 100; i++)
				{
					if (params->pool[i] == NULL)
					{
						...
						params->pool[i] = sharpenImage(params->pool[req.request.img_id]);
					}
				}
			}
		}

		if (req.request.img_op == IMG_VERTEDGES)
		{

			if (req.request.overwrite == 1)
			{
				struct image *new = detectVerticalEdges(params->pool[req.request.img_id]);
				deleteImage(params->pool[req.request.img_id]);
				params->pool[req.request.img_id] = new;
			}
			else if (req.request.overwrite == 0)
			{
				for (int i = 0; i < 100; i++)
				{
					if (params->pool[i] == NULL)
					{
						...
						params->pool[i] = detectVerticalEdges(params->pool[req.request.img_id]);
					}
				}
			}
		}

		if (req.request.img_op == IMG_HORIZEDGES)
		{

			if (req.request.overwrite == 1)
			{
				struct image *new = detectHorizontalEdges(params->pool[req.request.img_id]);
				deleteImage(params->pool[req.request.img_id]);
				params->pool[req.request.img_id] = new;
			}
			else if (req.request.overwrite == 0)
			{
				for (int i = 0; i < 100; i++)
				{
					if (params->pool[i] == NULL)
					{
						...
						params->pool[i] = detectHorizontalEdges(params->pool[req.request.img_id]);
					}
				}
			}
		}
										...
										struct worker_params *common_params)
{
	...
	if (cmd == WORKERS_START)
	{
		...
		for (i = 0; i < worker_count; ++i)
		{
			...
			worker_params[i]->pool = common_params->pool;
		}
...
void handle_connection(int conn_socket, struct connection_params conn_params)
{
	...
	struct image **images = (struct image **)malloc(100 * sizeof(struct image *));
	...
	for (int i = 0; i < 100; i++)
	{
		images[i] = (struct image *)malloc(sizeof(struct image));
		if (images[i] != NULL)
		{
			// Initialize the image or set its fields to default values
			images[i]->pixels = NULL; // For example, set pixels to NULL or allocate pixel memory too
		}
	}
			...
			if (req->request.img_op == IMG_REGISTER)
			{
				for (int i = 0; i < 100; i++)
				{
					if (images[i] == NULL)
					{
						...
						images[i] = recvImage(conn_socket);
