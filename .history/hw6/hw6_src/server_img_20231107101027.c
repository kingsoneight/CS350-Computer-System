	...
	while (!params->worker_done)
	{
		...
		if (req.request.img_op == IMG_ROT90CLKW)
		{
			...
			if (req.request.overwrite == 1)
			{
				...
				uint8_t errorStatus;
				struct image *new = rotate90Clockwise(images[req.request.img_id], errorStatus);
				deleteImage(images[req.request.img_id]);
				images[req.request.img_id] = new;
			...
			else if (req.request.overwrite == 0)
			{
				for (int i = 0; i < 100; i++)
				{
					if (images[i] == NULL)
					{
						...
						uint8_t errorStatus;
						images[i] = rotate90Clockwise(images[req.request.img_id], errorStatus);
						if (errorStatus == 0)
						{
							resp.img_id = i;
							break;
		...
		if (req.request.img_op == IMG_BLUR)
		{
			...
			if (req.request.overwrite == 1)
			{
				...
				struct image *new = blurImage(images[req.request.img_id]);
				deleteImage(images[req.request.img_id]);
				images[req.request.img_id] = new;
				resp.img_id = req.request.img_id;
			}
			else if (req.request.overwrite == 0)
			{
				for (int i = 0; i < 100; i++)
				{
					if (images[i] == NULL)
					{
						...
						images[i] = blurImage(images[req.request.img_id]);
						resp.img_id = i;
						break;
					}
				}
		...
		if (req.request.img_op == IMG_SHARPEN)
		{
			...
			if (req.request.overwrite == 1)
			{
				struct image *new = sharpenImage(images[req.request.img_id]);
				deleteImage(images[req.request.img_id]);
				images[req.request.img_id] = new;
				resp.img_id = req.request.img_id;
			}
			else if (req.request.overwrite == 0)
			{
				for (int i = 0; i < 100; i++)
				{
					if (images[i] == NULL)
					{
						...
						images[i] = sharpenImage(images[req.request.img_id]);
						resp.img_id = i;
						break;
					}
				}
		...
		if (req.request.img_op == IMG_VERTEDGES)
		{
			...
			if (req.request.overwrite == 1)
			{
				struct image *new = detectVerticalEdges(images[req.request.img_id]);
				deleteImage(images[req.request.img_id]);
				images[req.request.img_id] = new;
				resp.img_id = req.request.img_id;
			}
			else if (req.request.overwrite == 0)
			{
				for (int i = 0; i < 100; i++)
				{
					if (images[i] == NULL)
					{
						...
						images[i] = detectVerticalEdges(images[req.request.img_id]);
						resp.img_id = i;
						break;
					}
				}
		...
		if (req.request.img_op == IMG_HORIZEDGES)
		{
			...
			if (req.request.overwrite == 1)
			{
				struct image *new = detectHorizontalEdges(images[req.request.img_id]);
				deleteImage(images[req.request.img_id]);
				images[req.request.img_id] = new;
				resp.img_id = req.request.img_id;
			}
			else if (req.request.overwrite == 0)
			{
				for (int i = 0; i < 100; i++)
				{
					if (images[i] == NULL)
					{
						...
						images[i] = detectHorizontalEdges(images[req.request.img_id]);
						resp.img_id = i;
						break;
					}
				}
		...
		if (req.request.img_op == IMG_RETRIEVE)
		{
			sendImage(images[req.request.img_id], params->conn_socket);
		}

		/* IMPLEMENT ME! Print out the post-processing status
		 * report. */
		sync_printf("T%d R%ld:%lf,%s,%d,%d,%d,%lf,%lf,%lf\n",
								params->worker_id,
								req.request.req_id,
			...
			if (req->request.img_op == IMG_REGISTER)
			{
				...
				for (int i = 0; i < 100; i++)
				{
					if (images[i] == NULL)
					{
						...
						sync_printf("T0 R%ld:%lf,%s,%d,%d,%d,%lf,%lf,%lf\n",
												req->request.req_id,
												TSPEC_TO_DOUBLE(req->request.req_timestamp),
												OPCODE_TO_STRING(req->request.img_op),
												req->request.overwrite,
