...
observed_jobs_run1 = {i: resp_times(run1, i) for i in [
    'IMG_REGISTER', 'IMG_ROT90CLKW', 'IMG_BLUR', 'IMG_SHARPEN', 'IMG_VERTEDGES', 'IMG_HORIZEDGES']}

observed_jobs_run2 = {i: resp_times(run2, i) for i in [
    'IMG_REGISTER', 'IMG_ROT90CLKW', 'IMG_BLUR', 'IMG_SHARPEN', 'IMG_VERTEDGES', 'IMG_HORIZEDGES']}


# Define a dictionary to store the EWMA estimators for each request type
ewma_estimators = {}
# Define the smoothing factor alpha
alpha = 0.7

# Function to update the EWMA estimator for a given request type with a new job length


def update_ewma(request_type, observed_job_length):

    if request_type not in ewma_estimators:
        ewma_estimators[request_type] = observed_job_length
    else:
        # Calculate the new EWMA estimator value
        ewma_estimators[request_type] = (
            alpha * observed_job_length +
            (1 - alpha) * ewma_estimators[request_type]
        )

# Function to calculate misprediction error


def calculate_misprediction_error(prediction, actual_job_length):
    return abs(prediction - actual_job_length)


misprediction_errors = {}

for request_type, lengths in observed_jobs_run1.items():
    # Initialize the misprediction tracking for this request type
    misprediction_errors[request_type] = {'error_sum': 0, 'count': 0}

    # Initialize a variable to store the last prediction for this request type
    last_prediction = 0

    # Loop through each job length for this request type
    for job_length in lengths:
        # Update the EWMA estimator with the new observation
        update_ewma(request_type, job_length)

        # Calculate the misprediction error using the last prediction
        error = calculate_misprediction_error(last_prediction, job_length)

        # Update the sum of errors and count
        misprediction_errors[request_type]['error_sum'] += error
        misprediction_errors[request_type]['count'] += 1

        # The current estimate becomes the last prediction for the next iteration
        last_prediction = ewma_estimators[request_type]

# Compute the average misprediction error for each request type
average_misprediction_errors = {
    request_type: error_info['error_sum'] / error_info['count']
    for request_type, error_info in misprediction_errors.items()
}

# Output the average misprediction errors
for request_type, avg_error in average_misprediction_errors.items():
    print(f"Average misprediction error for {request_type}: {avg_error:.2f}")
