...
def group_output_by_imgop(client_report):
    ...
    for line in client_lines:
        opt = line.split('Opcode: ')[1]
        test += opt
    return test


# input values from above function and the client report, output dictionary with values being list of grouped reports
def group_client_outputs_by_thread(client_data):
    """
    ...
    for line in client_lines:
        # Extracting the request number
        request = f"R{line.split(' ')[1].split(':')[0][2:-1]}"
...
print(group_output_by_imgop(run2)[:15])
